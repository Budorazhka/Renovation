...
export type RealtorsDirectoryState =
  | { status: 'loading' }
  | { status: 'empty' }
  | { status: 'ready'; items: PublicRealtorProfile[]; nextCursor: string | null; loadingMore: boolean; loadMoreError: string | null }
  | { status: 'error'; message: string; statusCode?: number; retry: () => void }

/** N-13: та же форма, что useBuyerRequestsBoard/useListingsCatalogue — cursor pagination поверх публичного каталога риэлторов. */
export function useRealtorsDirectory(query: { city?: string } = {}): {
  state: RealtorsDirectoryState
  loadMore: () => void
  retryLoadMore: () => void
} {
  const { t } = useI18n()
  const [state, setState] = useState<RealtorsDirectoryState>({ status: 'loading' })
  ...
  const loadFirstPage = useCallback(async () => {
    ...
    try {
      const response = await marketplaceApi.listPublicRealtors({ city: city?.trim() || undefined }, { signal: controller.signal })
  ...
  const executeLoadMore = useCallback(() => {
    ...
    void marketplaceApi
      .listPublicRealtors({ city: city?.trim() || undefined, cursor }, { signal: controller.signal })
      .then(
        (response) => {
          if (requestIdRef.current !== requestId || controller.signal.aborted) return
          nextCursorRef.current = response.nextCursor
          setState((current) =>
            current.status !== 'ready'
              ? current
              : { status: 'ready', items: [...current.items, ...response.items], nextCursor: response.nextCursor, loadingMore: false, loadMoreError: null },
          )
        },
        (cause: unknown) => {
          if (requestIdRef.current !== requestId || controller.signal.aborted || isAbortError(cause)) return
