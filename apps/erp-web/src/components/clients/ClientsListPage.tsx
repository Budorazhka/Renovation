...
const ON_PRIMARY = 'var(--gold-btn-text)'
    ...
    <DashboardShell hideSidebar>
      ...
      >
        ...
        >
          <div style={{ flex: 1, position: 'relative', minWidth: 0 }}>
            ...
            <input
              ...
              placeholder={t('clients.clientsListPage.поиск_по_имени_телеф')}
              style={{
                width: '100%',
                height: 38,
          ...
          >
            ...
            {t('clients.clientsListPage.фильтры')}</button>
        </div>
        ...
        {filtersOpen && (
          ...
          >
            {t('clients.clientsListPage.расширенные_фильтры')}</div>
        )}
          ...
          {filtered.length > 0 && (
            ...
            >
              <p style={{ margin: 0, fontSize: 11, color: 'var(--workspace-text-dim)' }}>
                {t('clients.clientsListPage.показано')}{startIdx}–{endIdx} {t('clients.clientsListPage.из')}{filtered.length.toLocaleString('ru-RU')} {t('clients.clientsListPage.записей')}</p>
        ...
        >
          ...
          <div style={{ display: 'flex', minWidth: 0, minHeight: BENTO_TILE_MIN_PX }}>
            <BentoCard
              ...
              value="Бизнес-класс"
              sub={<span style={{ fontSize: 12, color: 'var(--workspace-text-muted)', marginTop: 4, display: 'block' }}>{t('clients.clientsListPage.68_новых_заявок_за_н')}</span>}
...
function BatumiSegmentsHoverDetail() {
  ...
  return (
    <div>
      ...
      >
        {t('clients.clientsListPage.батуми_новостройки_м')}</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {rows.map(r => (
          ...
          >
            <div>
              <div style={{ fontSize: 15, fontWeight: 400, color: 'var(--app-text)' }}>{r.seg}</div>
              {r.note && (
                <div style={{ fontSize: 13, color: 'var(--hub-body)', marginTop: 4, lineHeight: 1.4 }}>{r.note}</div>
              )}
            </div>
            <div style={{ fontSize: 15, fontWeight: 400, color: PRIMARY, whiteSpace: 'nowrap', textAlign: 'right' }}>{r.range}</div>
          </div>
        ))}
      </div>
      <p style={{ margin: '16px 0 0', fontSize: 12, color: 'var(--app-text-muted)', lineHeight: 1.45 }}>
        {t('clients.clientsListPage.ориентиры_по_обзорам')}</p>
    </div>
  )
}
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <span style={{ fontSize: 10, fontWeight: 400, color: PRIMARY, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
              {kicker}
            </span>
            {icon}
          </div>
          <h4 style={{ margin: 0, fontSize: 20, fontWeight: 400, color: 'var(--app-text)', lineHeight: 1.2 }}>{value}</h4>
