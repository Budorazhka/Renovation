    ...
    <DashboardShell>
      <div className="min-h-0 flex-1 overflow-y-auto p-6">
        <div className="w-full space-y-4">
          <div>
            <h1 className="text-xl font-normal text-[color:var(--theme-accent-heading)]">{t('newbuild.primaryPartnersReportPage.отч_т_о_работе_партн')}</h1>
            <p className="mt-1 text-sm text-[color:var(--app-text-muted)]">
              {t('newbuild.primaryPartnersReportPage.воронка_регистрация')}</p>
          </div>

          <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3 text-sm text-[color:var(--workspace-text-muted)]">
            <p className="text-[color:var(--workspace-text)]">
              {t('newbuild.primaryPartnersReportPage.фиксация_клиента_у_з')}{sessionRegs > 0 ? (
                <>
                  {' '}
                  {t('newbuild.primaryPartnersReportPage.в_этой_сессии_вы_уже')}{' '}
                  <span className="font-medium text-[color:var(--gold)]">{sessionRegs}</span> {t('newbuild.primaryPartnersReportPage.заявок_они_учитывают')}</>
              ) : (
                <> {t('newbuild.primaryPartnersReportPage.создайте_первую_заяв')}</>
              )}
            </p>
            <Link
              to="/dashboard/new-buildings/registration"
              className="mt-2 inline-flex text-sm font-normal text-[color:var(--gold)] underline-offset-2 hover:underline"
            >
              {t('newbuild.primaryPartnersReportPage.перейти_к_регистраци')}</Link>
          </section>
          ...
          <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('newbuild.primaryPartnersReportPage.фильтры')}</h2>
            </div>
            <div className="grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-4">
              ...
              >
                <option value="all">{t('newbuild.primaryPartnersReportPage.регион_все')}</option>
                {regionOptions.map((r) => (
              ...
              <select
                ...
                className="rounded-md border border-[var(--hub-card-border)] bg-[color-mix(in_srgb,var(--rail-bg)_82%,transparent)] px-2 py-2 text-sm text-[color:var(--workspace-text)] [color-scheme:dark]"
              >
                <option value="all">{t('newbuild.primaryPartnersReportPage.партн_р_все')}</option>
              ...
              <select
                ...
                className="rounded-md border border-[var(--hub-card-border)] bg-[color-mix(in_srgb,var(--rail-bg)_82%,transparent)] px-2 py-2 text-sm text-[color:var(--workspace-text)] [color-scheme:dark]"
              >
                <option value="4w">{t('newbuild.primaryPartnersReportPage.динамика_4_недели')}</option>
                <option value="12w">{t('newbuild.primaryPartnersReportPage.динамика_12_недель')}</option>
                <option value="all">{t('newbuild.primaryPartnersReportPage.динамика_весь_период')}</option>
              </select>
              <label className="flex cursor-pointer items-center gap-2 rounded-md border border-[var(--hub-card-border)] bg-[color-mix(in_srgb,var(--rail-bg)_82%,transparent)] px-2 py-2 text-sm text-[color:var(--workspace-text)] [color-scheme:dark]">
                ...
                />
                {t('newbuild.primaryPartnersReportPage.низкая_конверсия_бро')}</label>
            </div>
          </section>
          ...
          <section className="grid grid-cols-2 gap-2 md:grid-cols-3 lg:grid-cols-5">
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase tracking-wide text-[color:var(--app-text-subtle)]">{t('newbuild.primaryPartnersReportPage.регистрации')}</p>
              <p className="text-xl font-normal text-blue-400">{kpi.registrations}</p>
            </div>
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase tracking-wide text-[color:var(--app-text-subtle)]">{t('newbuild.primaryPartnersReportPage.брони')}</p>
              <p className="text-xl font-normal text-amber-300">{kpi.bookings}</p>
            </div>
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase tracking-wide text-[color:var(--app-text-subtle)]">{t('newbuild.primaryPartnersReportPage.сделки')}</p>
              <p className="text-xl font-normal text-emerald-400">{kpi.deals}</p>
            </div>
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase tracking-wide text-[color:var(--app-text-subtle)]">{t('newbuild.primaryPartnersReportPage.бронь_сделка')}</p>
              <p className="text-xl font-normal text-[color:var(--workspace-text)]">{kpi.convBook}%</p>
            </div>
            <div className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <p className="text-[10px] uppercase tracking-wide text-[color:var(--app-text-subtle)]">{t('newbuild.primaryPartnersReportPage.рег_сделка')}</p>
              <p className="text-xl font-normal text-[color:var(--workspace-text)]">{kpi.convReg}%</p>
            </div>
          </section>
          ...
          <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
            <div className="mb-3 flex items-center gap-2">
              ...
              <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('newbuild.primaryPartnersReportPage.детализация_по_партн')}</h2>
            </div>
              ...
              <table className="w-full min-w-[860px] border-collapse text-sm">
                <thead>
                  <tr className="border-b border-[color:var(--workspace-row-border)] text-left text-[11px] uppercase tracking-wide text-[color:var(--app-text-subtle)]">
                    <th className="px-2 py-2">{t('newbuild.primaryPartnersReportPage.партн_р')}</th>
                    <th className="px-2 py-2">{t('newbuild.primaryPartnersReportPage.проект')}</th>
                    <th className="px-2 py-2">{t('newbuild.primaryPartnersReportPage.регион')}</th>
                    <th className="px-2 py-2">{t('newbuild.primaryPartnersReportPage.регистрации')}</th>
                    <th className="px-2 py-2">{t('newbuild.primaryPartnersReportPage.брони')}</th>
                    <th className="px-2 py-2">{t('newbuild.primaryPartnersReportPage.сделки')}</th>
                    <th className="px-2 py-2">{t('newbuild.primaryPartnersReportPage.бронь_сделка')}</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
            <div className="mb-3 flex items-center gap-2">
              <BarChart3 className="size-4 text-[color:var(--gold)]" />
              <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('newbuild.primaryPartnersReportPage.динамика_агрегат_сет')}</h2>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[640px] border-collapse text-sm">
                <thead>
                  <tr className="border-b border-[color:var(--workspace-row-border)] text-left text-[11px] uppercase tracking-wide text-[color:var(--app-text-subtle)]">
                    <th className="px-2 py-2">{t('newbuild.primaryPartnersReportPage.неделя')}</th>
                    <th className="px-2 py-2">{t('newbuild.primaryPartnersReportPage.регистрации')}</th>
                    <th className="px-2 py-2">{t('newbuild.primaryPartnersReportPage.брони')}</th>
                    <th className="px-2 py-2">{t('newbuild.primaryPartnersReportPage.сделки')}</th>
                    <th className="px-2 py-2 min-w-[160px]">{t('newbuild.primaryPartnersReportPage.регистрации')}</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
            <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <div className="mb-2 flex items-center gap-2">
                ...
                <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('newbuild.primaryPartnersReportPage.просадка_конверсии')}</h2>
              </div>
              {problematic.length === 0 ? (
                <p className="text-sm text-[color:var(--app-text-muted)]">{t('newbuild.primaryPartnersReportPage.в_выборке_нет_строк')}</p>
              ) : (
                <ul className="space-y-2">
                  {problematic.map((r) => {
                    ...
                    return (
                      ...
                      >
                        {r.partner} · {r.project} {t('newbuild.primaryPartnersReportPage.конверсия')}{c}%
                      </li>
                    )
                  })}
                </ul>
            ...
            <section className="rounded-lg border border-[var(--hub-card-border)] bg-[var(--hub-card-bg)] p-3">
              <div className="mb-2 flex items-center gap-2">
                ...
                <h2 className="text-sm font-normal text-[color:var(--theme-accent-heading)]">{t('newbuild.primaryPartnersReportPage.топ_по_сделкам')}</h2>
              </div>
              <ul className="space-y-2">
                {[...filteredRows]
