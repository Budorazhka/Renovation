...
export interface Lead {
  ...
  source: LeadSource
  stageId: LeadStageId
  /** ID менеджера, которому назначен лид (если уже распределён). Легаси-поле: id человека. */
  managerId: string | null
  /**
   * Владелец-ПОЗИЦИЯ лида (Stage A): стабильный positionId, не меняется при смене человека.
   * Заполняется бэкендом; на фронте резолвится из managerId через positionOf().
   */
  ownerPositionId?: string
  /** Кто фактически ведёт лид сейчас — снимок аккаунта текущего занимающего позицию. */
...
/** Пользователь с доступом к рабочему столу лидов — задаётся по email личного кабинета */
