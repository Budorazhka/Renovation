...
- [erp-reminders-and-password.md](operations/erp-reminders-and-password.md) — смена пароля с закрытием прочих сессий, напоминания на событиях календаря, виджеты второго экрана и база знаний в чатах на данных сервера.
- [client-registrations.md](operations/client-registrations.md) — фиксация клиента у застройщика: заявка агентства, решение застройщика, закрепление на шесть месяцев и защита от второй заявки на того же человека.
- [referral-network.md](operations/referral-network.md) — реферальная сеть BAZA: кураторы по ссылке, команды одного уровня, 7% от комиссии по первичке, отметка денег и выплаты в админке, дерево сети в админке, кабинете маркетплейса и ERP.
