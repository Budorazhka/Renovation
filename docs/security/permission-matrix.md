...
| «Имена людей» | `person.rename.global` — сменить человеку имя в должности (`PATCH /admin/people/:identityId`), причина обязательна и уходит в аудит `person.rename` со старым и новым именем. Имя после регистрации меняет BAZA, не сам человек (решение владельца 16.09.2026) |
